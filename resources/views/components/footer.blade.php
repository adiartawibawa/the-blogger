<footer class="bg-stone-100 dark:bg-stone-900 border-t border-stone-200 dark:border-stone-800 mt-24">
    <div class="max-w-6xl mx-auto px-6 lg:px-8 py-12">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            ...
            <div>
                <div class="flex items-center gap-2 mb-3">
                    <div class="w-7 h-7 bg-indigo-600 dark:bg-indigo-500 rounded-md flex items-center justify-center">
                        <span
                            class="text-white font-mono font-bold text-xs">{{ strtoupper(substr(config('portfolio.owner_name', 'D'), 0, 1)) }}</span>
                    </div>
                    <span
                        class="font-mono font-bold text-stone-900 dark:text-stone-100">{{ config('portfolio.owner_name', 'DevPortfolio') }}</span>
                </div>
            ...
            {{-- Quick Links --}}
            <div>
                <h4 class="font-semibold text-stone-900 dark:text-stone-100 text-sm mb-3">Navigasi</h4>
                <ul class="space-y-2">
            ...
            {{-- Social Links --}}
            <div>
                <h4 class="font-semibold text-stone-900 dark:text-stone-100 text-sm mb-3">Connect</h4>
                <div class="flex gap-3">
                    ...
                    @if (config('portfolio.social.twitter'))
                        <a href="{{ config('portfolio.social.twitter') }}" target="_blank" rel="noopener noreferrer"
                            class="w-9 h-9 bg-stone-200 dark:bg-stone-800 rounded-lg flex items-center justify-center text-stone-600 dark:text-stone-400 hover:bg-sky-100 dark:hover:bg-sky-900/40 hover:text-sky-600 dark:hover:text-sky-400 transition-all">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                <path
        ...
        <div
            class="border-t border-stone-200 dark:border-stone-800 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p class="text-stone-400 dark:text-stone-500 text-xs font-mono">
